...
/*** ZTEMT start ,add palm_sleep gesture,20150103***/
#define ZTEMT_CYPRESS_PALM_SLEEP    1
/*ZTEMT end*/
...
{
	struct cyttsp5_mt_data *md = NULL;
	md = &cd->md;

	dev_dbg(md->dev, "%s: md=%p  md->si=%p\n", __func__, md, md->si);
	...
	struct cyttsp5_touch *touch)
{
	...
	/*** ZTEMT start ***/
    //dev_err(dev, "%s: ZTEMT flags=%ud \n",__func__,md->pdata->flags);
    md->pdata->flags = 0x00;
    /*ZTEMT end*/
...
/*** ZTEMT start ,add palm_sleep gesture,20150103***/
	...
	if(!fw_updating){
        ...
        //input_report_abs(md->input, ABS_MT_POSITION_X, 0);
        //input_report_abs(md->input, ABS_MT_POSITION_Y, 0);
...
static void cyttsp5_mt_send_dummy_event(struct cyttsp5_mt_data *md)
{
	...
	switch (gesture_id) {
		...
		case GESTURE_LETTER_w:
			dev_err(md->dev, "Letter W.\n");
			input_report_key(md->input, KEY_F5, 1);
			input_sync(md->input);
			input_report_key(md->input, KEY_F5, 0);
			input_sync(md->input);
			break;
		case GESTURE_LETTER_m:
			dev_err(md->dev, "Letter M.\n");
			input_report_key(md->input, KEY_F6, 1);
			input_sync(md->input);
			input_report_key(md->input, KEY_F6, 0);
			input_sync(md->input);
			break;
		case GESTURE_LETTER_e:
			dev_err(md->dev, "Letter E.\n");
			input_report_key(md->input, KEY_F7, 1);
			input_sync(md->input);
			input_report_key(md->input, KEY_F7, 0);
			input_sync(md->input);
			break;
		case GESTURE_LETTER_c:
			dev_err(md->dev, "Letter C.\n");
			input_report_key(md->input, KEY_F8, 1);
			input_sync(md->input);
			input_report_key(md->input, KEY_F8, 0);
			input_sync(md->input);
			break;
		case GESTURE_LETTER_o:
			dev_err(md->dev, "Letter O.\n");
			input_report_key(md->input, KEY_F9, 1);
			input_sync(md->input);
			input_report_key(md->input, KEY_F9, 0);
			input_sync(md->input);
			break;

        case GESTURE_LETTER_v:
            dev_err(md->dev, "Letter v.\n");
            input_report_key(md->input, KEY_F11, 1);
            input_sync(md->input);
            input_report_key(md->input, KEY_F11, 0);
            input_sync(md->input);
            break;

        case GESTURE_LETTER_z:
            dev_err(md->dev, "Letter z.\n");
		...
		case GESTURE_SLIDE_NORTH:
			dev_err(md->dev, "Letter O.\n");
			input_report_key(md->input, KEY_F1, 1);
			input_sync(md->input);
			input_report_key(md->input, KEY_F1, 0);
			input_sync(md->input);
			break;
		case GESTURE_SLIDE_EAST:
			dev_err(md->dev, "Letter O.\n");
			input_report_key(md->input, KEY_F2, 1);
			input_sync(md->input);
			input_report_key(md->input, KEY_F2, 0);
			input_sync(md->input);
			break;
		case GESTURE_SLIDE_SOUTH:
			dev_err(md->dev, "Letter O.\n");
			input_report_key(md->input, KEY_F3, 1);
			input_sync(md->input);
			input_report_key(md->input, KEY_F3, 0);
			input_sync(md->input);
			break;
		case GESTURE_SLIDE_WEST:
			dev_err(md->dev, "Letter O.\n");
			input_report_key(md->input, KEY_F4, 1);
			input_sync(md->input);
			input_report_key(md->input, KEY_F4, 0);
			input_sync(md->input);
			break;
		default:
			dev_err(md->dev, "wrong gesture id\n");
			break;
	}
#endif
}
...
static int cyttsp5_setup_input_device(struct device *dev)
{
	...
	__set_bit(KEY_POWER, md->input_rim->keybit);
