	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	if(!strncmp(slave_info->sensor_name,"t4k37_qtech_f4k37ab_msm8939",30)||!strncmp(slave_info->sensor_name,"ov5648_msm8939",20)){
	s_ctrl->sensordata->power_info.cam_vreg[0].min_voltage=2000000;
	s_ctrl->sensordata->power_info.cam_vreg[0].max_voltage=2100000;
	pr_err("wdy after change %s  min_DVDD_s4=%d  max_DVDD_s4=%d\n",__func__,\
		s_ctrl->sensordata->power_info.cam_vreg[0].min_voltage,s_ctrl->sensordata->power_info.cam_vreg[0].max_voltage);
	pr_err("wdy  changed DVDD  %s  of  current_sensor_name=%s\n",__func__,slave_info->sensor_name);
	}
